<?php
    ...
    require 'conexao.php';   // Conexão com o banco de dados (mysqli)
    ...
    // Atualiza as notificações para marcar como lidas
    $sqlUpdate = "UPDATE notificacoes SET lida = 1 WHERE id_cliente = $id_cliente AND lida = 0";
    $conexao->query($sqlUpdate);

    // Recupera as notificações do cliente
    $sqlNotifs = "SELECT * FROM notificacoes WHERE id_cliente = $id_cliente ORDER BY data DESC";
    $resultNotifs = $conexao->query($sqlNotifs);
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <title>Minhas Notificações - Geovane Services</title>
    <!-- Bootstrap 5 CSS -->
   ...
   <div class="topbar">
    ...
    <div class="content">
        <div class="container">
            ...
            <?php if ($resultNotifs->num_rows > 0): ?>
                <ul class="list-group">
                    <?php while($notif = $resultNotifs->fetch_assoc()): ?>
                        <li class="list-group-item <?php echo ($notif['lida'] == 0) ? 'list-group-item-warning' : ''; ?>">
                            <div class="d-flex justify-content-between align-items-center">
                                <span><?php echo htmlspecialchars($notif['mensagem']); ?></span>
